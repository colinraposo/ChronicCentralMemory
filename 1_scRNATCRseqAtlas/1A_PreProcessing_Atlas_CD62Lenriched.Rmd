# Pre-processing for scRNA/TCR-seq of GP33+ cells from different timepoints after LCMV-Cl13 - batch 2 Gp33+ cells split by CD62L
...
```{r}
path <- "~/Experiments/CR119_scRNATCR_ChronMem"
...
## we also did scRNA-seq of Gp276 cells, but those data are not uploaded as we filtered out those lanes prior to integration
lanes <- c("001_GP33_62Lpos" , "002_GP276_62Lpos", "003_GP33_62Lneg" , "004_GP33_62Lneg", "005_GP33_62Lneg", "006_GP276_62Lneg", "007_GP276_62Lneg", "008_GP276_62Lneg")

# run function that creates a so for each 10X lane - x is the name of the lane
so.list <- lapply(lanes, function(x){

  # define lane_path for the inidividual lane
  gex_path <- paste0(path, "/00_CellRangerOuts/cell_ranger_gex/" , x)
  tcr_path <- paste0(path, "/00_CellRangerOuts/cell_ranger_tcr/" , x)
...
do.call(ggarrange , c(tcr.plots, common.legend = T, legend = "right" , ncol = 8))
...
```{r}
# calculate cutoffs for % mito and n feature - based on 2x SD for features and 95%ile for %mito
...
# check count of cells in each lane
lapply(filtered.so.list, function(so) {table(so$orig.ident)}) 
 

```

## function for demultiplexing with Demultiplex 
```{r}
...
# create read table ++++++++++
  ...
  ms.file.root <- files[grepl(name.id.name, files)][1] %>% # take first fastq file
...
bar.ref.full <- read.csv(paste0(path, "/00_CellRangerOuts/C119_Bartable.csv"))  #read in reference
bar.ref.full
...
## save RDS
```{r}
saveRDS(so.list.demuxed, file = paste0(path, "/99_seurat_objects/1A_DemuxedSeuratObjectList_batch2.rds"))
```
