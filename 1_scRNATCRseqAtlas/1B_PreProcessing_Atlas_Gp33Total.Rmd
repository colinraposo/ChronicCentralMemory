# Pre-processing for scRNA/TCR-seq of GP33+ cells from different timepoints after LCMV-Cl13 - batch 1 Gp33+ cells 
## pre-processing starts with outs from cellranger - available on Github and GEO (GSE285411)
...
## set paths
```{r}
path <- "~/Experiments/CR82_scRNATCR_Cl13Mem_vs_ArmMem_vsCl13"
...
```{r}
# x is the name of the lane
lanes <- c("GP33pos_1" , "GP33pos_2", "GP33pos_3" , "GP33neg_4")
...
so.list <- lapply(lanes, function(x){
  ...
  lane_path <- paste0(path, "/cell_ranger_outs_compiled/" , x)
  
  # generate counts matrix
  counts <- Read10X_h5(paste0(lane_path , "/filtered_feature_bc_matrix.h5"))
  
  # Append TCR information to metadata tables
  tcr_table <- fread(input = paste0(lane_path, "/filtered_contig_annotations.csv"))
  
  clonotype_table <- fread(input = paste0(lane_path, "/clonotypes.csv"))
...
## add percent MT and cell cycle to seurat object
```{r}
# add percent mt 
...
bar.ref.full <- read.csv(paste0(path, "/multiseq/CR82_Bartable.csv"))  #refernce for barcodes
bar.ref.full
...
tag.ref <- tag.ref[1:7] # choose only bars that we actually used - change this
nbar <- length(tag.ref)

ms.fastq <- paste0(path, "/multiseq/fastq_multiseq/") # path for fastq files

# create read table  ==========================================================================
read_table <- readTags(dir = ms.fastq,
                       name = "Multi_GP33pos_1_S9_L001", ### CHANGE THIS
...
# create read table  ==========================================================================
read_table <- readTags(dir = ms.fastq,
                       name = "Multi_GP33pos_2_S10_L001", ### CHANGE THIS
                       barcode.type = "MULTIseq",
                       assay = "RNA")
...
table(so_demuxed_lane2$multiseq_class)
...
# create read table  ==========================================================================
read_table <- readTags(dir = ms.fastq,
                       name = "Multi_GP33pos_3_S11_L001", ### CHANGE THIS
                       barcode.type = "MULTIseq",
                       assay = "RNA")
...
table(so_demuxed_lane3$multiseq_class)

```

# demultiplexing - lane 4
## we also did scRNA-seq of GP33negatice cells, but those data are not uploaded as we filtered out those lanes prior to integration
```{r}
library(deMULTIplex2)
library(deMULTIplex)
...
bar.ref.full <- read.csv(paste0(path, "/multiseq/CR82_Bartable.csv"))  #reference for bar codes
bar.ref.full
...
# create read table  ==========================================================================
read_table <- readTags(dir = ms.fastq,
                       name = "Multi_GP33neg_4_S12_L001", ### CHANGE THIS
                       barcode.type = "MULTIseq",
                       assay = "RNA")
...
table(so_demuxed_lane4$multiseq_class)

```

# create list of seurat objects and look at QC metrics
```{r}
so_demuxed_list <- list(so_demuxed_lane1, so_demuxed_lane2, so_demuxed_lane3, so_demuxed_lane4)
...
saveRDS(so_demuxed_list, file = paste0(path, "/seurat_objects/1B_DemuxedSeuratObjectList_batch1.rds"))
