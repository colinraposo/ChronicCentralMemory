# Pre-processing for scRNA/TCR-seq CD8 T cells from 8 dpi after LCMV-Arm rechallenge 
## pre-processing starts with outs from cellranger - available on Github and GEO (GSE285414)
...
## set paths for folders containing data and where to output data
```{r}
...
```{r}
# create a list of lane IDs for individual 10X reactions 
## note sample names got mixed up while submitting the sample sheet for sequencing - accidentally skipped GP33neg 3, data online are correctly numbered

lanes <- c("GP33pos_1" , "GP33neg_1", "GP33neg_2" , "GP33neg_3", "GP33neg_4", "GP33neg_5", "GP33neg_6")

# run function that creates a so for each 10X lane
so.list <- lapply(lanes, function(x){

  # define lane_path for the inidividual lane
  gex_path <- paste0(path, "/0_CellRangerOuts/cell_ranger_gex/" , x)
  tcr_path <- paste0(path, "/0_CellRangerOuts/cell_ranger_tcr/" , x)
  
  # generate counts matrix
  counts <- Read10X_h5(paste0(gex_path , "/filtered_feature_bc_matrix.h5"))
  
  # Append TCR information to metadata tables
  tcr_table <- fread(input = paste0(tcr_path, "/filtered_contig_annotations.csv"))
  
  clonotype_table <- fread(input = paste0(tcr_path, "/clonotypes.csv"))
...
do.call(ggarrange , c(tcr.plots, common.legend = T, legend = "right" , ncol = 4))
...
## note: this process of demuxing each lane can be sped up by utilizing lapply. See the notebooks from `1_scRNATCRseqAtlas` if you would like to see that pipeline
```{r}
library(deMULTIplex2)
library(deMULTIplex)

# set up for demux ==============================================================================
so <- filtered.so.list[[1]] # set SO

bar.ref.full <- read.csv(paste0(path, "/0_CellRangerOuts/CR96_Bartable.csv"))  #reference for barcodes
bar.ref.full
...
ms.fastq <- paste0(path, "/0_CellRangerOuts/ms_fastq/") # path for fastq files

# create read table  ==========================================================================
read_table <- readTags(dir = ms.fastq,
                       name = "MS_CR96_GP33pos_1_S7_L001", ### CHANGE THIS
...
ggsave(path = outs, filename = paste("MultiSeqTSNE_" , unique(so$orig.ident), ".pdf") , height = 4, width = 12)
write.csv(ms.data %>% dplyr::select(c(Cell_Barcode, Call)), 
          file = paste0(outs, "/MultiSeqCalls_" , unique(so$orig.ident), ".csv"))
write.csv(tag_mtx_filtered, file = paste0(outs, "/MultiSeqBarTable_" , unique(so$orig.ident), ".csv"))

# add to new SO  ==========================================================================

so_demuxed_lane1 <- AddMetaData(object = so, metadata = setNames(ms.data$Call, paste0(ms.data$Cell_Barcode, "-1")), col.name =  "multiseq_class") 

table(so_demuxed_lane1$multiseq_class)
...
# set up for demux ==============================================================================
so <- filtered.so.list[[2]] # set SO

head(so)

bar.ref.full <- read.csv(paste0(path, "/0_CellRangerOuts/CR96_Bartable.csv"))  #refernce for barcodes
bar.ref.full
...
# create read table  ==========================================================================
read_table <- readTags(dir = ms.fastq,
                       name = "MS_CR96_GP33neg_1_S8_L001", ### CHANGE THIS
                       barcode.type = "MULTIseq",
                       assay = "RNA")
...
so_demuxed_lane2 <- AddMetaData(object = so, metadata = setNames(ms.data$Call, paste0(ms.data$Cell_Barcode, "-1")), col.name =  "multiseq_class") 

```

# demultiplexing - lane 3
```{r}
library(deMULTIplex2)
library(deMULTIplex)

# set up for demux ==============================================================================
so <- filtered.so.list[[3]] # set SO
...
# create read table  ==========================================================================
read_table <- readTags(dir = ms.fastq,
                       name = "MS_CR96_GP33neg_2_S9_L001", ### CHANGE THIS
                       barcode.type = "MULTIseq",
                       assay = "RNA")
...
so_demuxed_lane3 <- AddMetaData(object = so, metadata = setNames(ms.data$Call, paste0(ms.data$Cell_Barcode, "-1")), col.name =  "multiseq_class") 

```

# demultiplexing - lane 4
```{r}
library(deMULTIplex2)
library(deMULTIplex)

# set up for demux ==============================================================================
so <- filtered.so.list[[4]] # set SO
...
# create read table  ==========================================================================
read_table <- readTags(dir = ms.fastq,
                       name = "MS_CR96_GP33neg_4_S18_L003", ### CHANGE THIS
                       barcode.type = "MULTIseq",
                       assay = "RNA")
...
so_demuxed_lane4 <- AddMetaData(object = so, metadata = setNames(ms.data$Call, paste0(ms.data$Cell_Barcode, "-1")), col.name =  "multiseq_class") 


```

# demultiplexing - lane 5
```{r}
library(deMULTIplex2)
library(deMULTIplex)

# set up for demux ==============================================================================
so <- filtered.so.list[[5]] # set SO
...
# create read table  ==========================================================================
read_table <- readTags(dir = ms.fastq,
                       name = "MS_CR96_GP33neg_5_S19_L003", ### CHANGE THIS
                       barcode.type = "MULTIseq",
                       assay = "RNA")
...
so_demuxed_lane5 <- AddMetaData(object = so, metadata = setNames(ms.data$Call, paste0(ms.data$Cell_Barcode, "-1")), col.name =  "multiseq_class") 


```

# demultiplexing - lane 6
```{r}
library(deMULTIplex2)
library(deMULTIplex)

# set up for demux ==============================================================================
so <- filtered.so.list[[6]] # set SO
...
# create read table  ==========================================================================
read_table <- readTags(dir = ms.fastq,
                       name = "MS_CR96_GP33neg_6_S20_L003", ### CHANGE THIS
                       barcode.type = "MULTIseq",
                       assay = "RNA")
...
so_demuxed_lane6 <- AddMetaData(object = so, metadata = setNames(ms.data$Call, paste0(ms.data$Cell_Barcode, "-1")), col.name =  "multiseq_class") 


```

# demultiplexing - lane 7
```{r}
library(deMULTIplex2)
library(deMULTIplex)

# set up for demux ==============================================================================
so <- filtered.so.list[[7]] # set SO
...
# create read table  ==========================================================================
read_table <- readTags(dir = ms.fastq,
                       name = "MS_CR96_GP33neg_7_S21_L003", ### CHANGE THIS
                       barcode.type = "MULTIseq",
                       assay = "RNA")
...
so_demuxed_lane7 <- AddMetaData(object = so, metadata = setNames(ms.data$Call, paste0(ms.data$Cell_Barcode, "-1")), col.name =  "multiseq_class") 

```

# create list of seurat objects and visualize QC metrics
```{r}
so_demuxed_list <- list(so_demuxed_lane1, so_demuxed_lane2, so_demuxed_lane3, so_demuxed_lane4, so_demuxed_lane5, so_demuxed_lane6, so_demuxed_lane7)

qc_metrix <- lapply(so_demuxed_list, function(x) x[[c("orig.ident", "nFeature_RNA", "nCount_RNA", "percent.mt", "Phase" , "chains", "multiseq_class")]])
...
qc_metrix %>% ggplot(aes(x = multiseq_class, fill = chains)) + 
  ...
  rotate_x_text(45) + scale_fill_manual(values = tcr_chains_pal) + 
...
ggplot(qc_metrix, aes(x=percent.mt, y = nFeature_RNA))  + 
  ...
  facet_wrap(~orig.ident) + #scale_x_log10() +
  scale_color_manual(values = brewer.pal(12, "Paired"), name = "Multiseq Class") 

table(qc_metrix$orig.ident, qc_metrix$multiseq_class)
```
# save RDS
```{r}
saveRDS(so_demuxed_list, file = paste0(path, "/seurat_objects/7A_DemuxedSeuratObjectList.rds"))
```
